...
public class Header {
	...
	public void serializerHeader(XmlSerializer serializer, String body){
		...
		String orInfo = time + ConstantValue.AGENTER_PASSWORD+body;
		String md5Hex = DigestUtils.md5Hex(orInfo);
		digest.setTagValue(md5Hex);
		
		try {
			serializer.startTag(null, "header");
			
			agenterid.serialize(serializer);
			source.serialize(serializer);
			compress.serialize(serializer);
			
			messengerid.serialize(serializer);
			timestamp.serialize(serializer);
			digest.serialize(serializer);
			
			transactiontype.serialize(serializer);
			username.serialize(serializer);
